...
class DynamicCatchEnvCfgV2(DirectRLEnvCfg):
    ...
    # D435
    D435 = RigidObjectCfg(
        prim_path="/World/envs/env_.*/D435",
        spawn=sim_utils.UsdFileCfg(
            usd_path=f"{ISAAC_NUCLEUS_DIR}/Robots/Franka/DetailedProps/realsense.usd",
            mass_props=sim_utils.MassPropertiesCfg(mass=0.1),
        ),
        init_state=RigidObjectCfg.InitialStateCfg(pos=(0.4, 0.0, 0.81),
                                                  rot=(0.0, 0.0, 0.0, 1.0))
    )
...
class DynamicCatchEnvV2(DirectRLEnv):
    ...
    cfg: DynamicCatchEnvCfgV2

    def __init__(self, cfg: DynamicCatchEnvCfgV2,
    ...
    def _compute_intermediate_values(self, env_ids: torch.Tensor | None = None):
        ...
        self.D435_image = self.scene["camera"].data.output["rgb"]
        import matplotlib.pyplot as plt

        # Save the D435 image
        for i in range(len(self.D435_image)):
            plt.imshow(self.D435_image[i].cpu())
