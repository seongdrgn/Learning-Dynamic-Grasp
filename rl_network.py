...
def shared_model_for_pretrain(observation_space: Optional[Union[int, Tuple[int], gym.Space, gymnasium.Space]] = None,
    ...
    class GaussianDeterministicModel(GaussianMixin, DeterministicMixin, Model):
        def __init__(self, observation_space, action_space, device, roles, metadata, single_forward_pass):
            ...
            self.robot_prop_net = nn.Sequential(nn.Linear(metadata[0]["robot_prop_input_shape"],metadata[0]["hiddens"][0]),
                                                nn.ELU(),
                                                nn.Linear(metadata[0]["hiddens"][0],metadata[0]["hiddens"][1]),
                                                nn.ELU(),
                                                )

            self.object_net = nn.Sequential(nn.Linear(metadata[0]["object_input_shape"]*metadata[0]["hist_t"],metadata[0]["hiddens"][0]),
                                            nn.ELU(),
                                            nn.Linear(metadata[0]["hiddens"][0],metadata[0]["hiddens"][1]),
                                            nn.ELU(),
                                            )
            ...
            print(_get_num_units_by_shape(self, metadata[0]["output_shape"]))
            mean_layers = [nn.Linear(metadata[0]["hiddens"][-1], 22)]
            ...
            value_layers = [nn.Linear(metadata[1]["hiddens"][-1], 1)]
        ...
        def compute(self, inputs, role):
            ...
            robot_prop = net_inputs[:,:29]
            object_pos = net_inputs[:,-12:]

            # single shared layers/network forward-pass
            if self._single_forward_pass:
                if role == self._roles[0]:
                    self._shared_output = torch.cat([self.robot_prop_net(robot_prop),self.object_net(object_pos)],dim=-1)
                    return self.instantiator_output_scales[0] * self.mean_net(self._shared_output), self.log_std_parameter, {}
                elif role == self._roles[1]:
                    shared_output = torch.cat([self.robot_prop_net(robot_prop),self.object_net(object_pos)],dim=-1) if self._shared_output is None else self._shared_output
                    self._shared_output = None
                    return self.instantiator_output_scales[1] * self.value_net(shared_output), {}
            # multiple shared layers/network forwrad-pass
            else:
                if role == self._roles[0]:
                    return self.instantiator_output_scales[0] * self.mean_net(torch.cat([self.robot_prop_net(robot_prop),self.object_net(object_pos)],dim=-1)), self.log_std_parameter, {}
                elif role == self._roles[1]:
                    return self.instantiator_output_scales[1] * self.value_net(torch.cat([self.robot_prop_net(robot_prop),self.object_net(object_pos)],dim=-1)), {}
...
def gaussian_model(observation_space: Optional[Union[int, Tuple[int], gym.Space, gymnasium.Space]] = None,
    ...
    class GaussianModel(GaussianMixin, Model):
        def __init__(self, observation_space, action_space, device, clip_actions,
            ...
            self.robot_prop_net = nn.Sequential(nn.Linear(metadata["robot_prop_input_shape"],128),
                                                nn.ELU(),
                                                nn.Linear(128,256),
                                                nn.ELU(),
                                                )

            self.object_net = nn.Sequential(nn.Linear(metadata["object_input_shape"],128),
                                            nn.ELU(),
                                            nn.Linear(128,256),
                                            nn.ELU(),
                                            )

            self.policy_net = nn.Linear(512,metadata["output_shape"])
        ...
        def compute(self, inputs, role=""):
            robot_prop_idx = metadata["robot_prop_input_shape"]
            robot_prop = inputs["states"][:,:robot_prop_idx]
            object_pos = inputs["states"][:,-metadata["object_input_shape"]:]
            if self.instantiator_input_type == 0:
